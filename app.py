...
from flask_cors import CORS  # Ապահովում է ռեալ տիրույթում կապը այլ կայքերի հետ

# Ավտոմատ գտնում է ներկա պապկայի ճանապարհը (դինամիկ հասցեներ)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
TEMPLATE_DIR = os.path.join(BASE_DIR, "templates")
IMAGE_FOLDER = os.path.join(BASE_DIR, "images")

app = Flask(__name__, template_folder=TEMPLATE_DIR)
CORS(app)  # Միացնում ենք CORS անվտանգությունը

# Ստեղծում ենք images պապկան, եթե այն գոյություն չունի սերվերում
...
def get_coordinates(city_name):
    ...
    try:
        response = requests.get(geo_url, timeout=5) # Ավելացված է timeout անվտանգության համար
...
if __name__ == '__main__':
    # Ռեալ սայթի համար debug-ը անջատվում է, և թույլատրվում է արտաքին կապեր (0.0.0.0)
