    ...
    if "," in city_name:
        try:
            ...
            if response.status_code == 200:
                ...
                if place_name:
                    return {"lat": lat, "lon": lon, "name": place_name, "country": country}
            
            return {"lat": lat, "lon": lon, "name": f"{lat}, {lon}", "country": "Ընտրված վայր"}
        except Exception as e:
            print(f"Reverse geocoding error: {e}")

    geo_url = f"https://geocoding-api.open-meteo.com/v1/search?name={city_name}&count=1&language=hy&format=json"
    try:
